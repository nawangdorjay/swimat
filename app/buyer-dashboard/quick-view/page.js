...
const ProductViewModal = ({
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      toast.success(`Order submitted! Order ID: ${result.data.orderId}. Payment verification pending.`);
  ...
  const handleBuyNow = () => {
    if (currentUserLoading) {
      toast("Please wait while we load your profile.");
      return;
    }
  ...
  const handleContactSeller = async () => {
    if (currentUserLoading) {
      toast("Your profile is still loading. Please wait a moment and try again.");
      return;
    }
    ...
    try {
      ...
      if (!conversationId) {
        ...
        toast.error("Conversation created but navigation failed. Please check your messages.");
        return;
      }
